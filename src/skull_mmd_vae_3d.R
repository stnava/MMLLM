...
read_lm_from_image <- function( x ) {
  temp = antsImageRead( x )
  return( getCentroids( temp )[,1:3] )
}
...
demog$isTrain = rep( FALSE, nrow(demog) )
demog$isTrain[istrain]=TRUE
...
if ( ! exists( "imgListTest" ) ) {
  ...
  ct = 1
  for ( k in mynums ) {
    if ( ct == 10 ) print(demog$fnimg[k])
    img = antsImageRead( demog$fnimg[k] )
    seg = antsImageRead( demog$fnseg[k] )
    pts = getCentroids( seg )[,1:img@dimension]
    print(min(rowSums(abs(pts))))
    if ( nrow( pts ) == 55  & min(rowSums(abs(pts))) > 0) {
      imgListTest[[ct]] = img
      ptListTest[[ct]] = pts
      cat(paste0(ct,"..."))
...
orinet =  createResNetModel3D(
       ...
       mode = "regression")
load_model_weights_hdf5( orinet, "models/mouse_orientation_3Db_GPU3.h5" )

if ( ! exists( "imgListTestReo" ) ) {
  load_model_weights_hdf5( orinet, "models/mouse_orientation_3Db_GPU3.h5" )
  imgListTestReo = list()
  ptListTestReo = list()
  ...
  wtest = 1:length(imgListTest)
  for ( k in wtest ) {
    print(paste("Begin",k ) )
    imgCoM = getCenterOfMass( iMath(imgListTest[[k]], "Normalize") )
    imgarr = array( as.array( iMath(imgListTest[[k]], "Normalize") ), dim=c(1,locdim,1) )
#    treg = antsRegistration( reoTemplate, iMath(imgListTest[[k]], "Normalize"), "Translation" )
#    imgarr = array( as.array( treg$warpedmovout ), dim=c(1,locdim,1) )
    ...
    rotated = applyAntsrTransformToImage( myaff, imgListTest[[k]], reoTemplate )
    qreg = antsRegistration( reoTemplate, imgListTest[[k]], "Rigid", initialTransform=myaff )
    print( paste(
      antsImageMutualInformation( reoTemplate, qreg$warpedmovout ),
      antsImageMutualInformation( reoTemplate, rotated ) ) )
    imgListTestReo[[k]] = qreg$warpedmovout
    ptListTestReo[[k]] = antsApplyTransformsToPoints( reoTemplate@dimension,
      ptListTest[[k]], qreg$fwdtransforms, whichtoinvert=TRUE )
    if ( FALSE ) {
      layout(matrix(1:4,nrow=1,byrow=T))
      plot( imgListTest[[k]],  alpha=0.5 )
      plot( reoTemplate,  alpha=0.5 )
      plot( qreg$warpedmovout,  alpha=0.5 )
      plot( reoTemplate, qreg$warpedmovout, alpha=0.5 )
      }
    }
  }


if ( ! exists( "imgListTestReo" ) & FALSE ) {
  templatenum=10
  reoTemplate = smoothImage( imgListTest[[templatenum]], 0.5, sigmaInPhysicalCoordinates=F )
  reoTemplate=iMath(reoTemplate,"Normalize")*255
  reoPts=ptListTest[[templatenum]]
  mypre = prepro( reoTemplate )
  fm1 = iMath( reoTemplate, "Canny", 3.5, 20, 40 ) %>% morphology("close",2)
  mask = randomMask( fm1, 1024 )
  imgListTestReo = list()
  ptListTestReo = list()
  for ( k in 1:length(imgListTest) ) {
    print(paste("Begin",k ) )
    dnz = smoothImage( imgListTest[[k]], 0.5, sigmaInPhysicalCoordinates=F )
    dnz = iMath(dnz,"Normalize")*255 # reg$warpedmovout
    dnzpt = ptListTest[[k]]
    fm2 = iMath( dnz, "Canny", 3.5, 20, 40 ) %>% morphology("close",2)
    nP2 = round( sum( fm2 == 1 )*0.015 )
    print(nP2)
    mask2 = randomMask( fm2, nP2 )
    psz = 16
#    psz = 8 # ripmmarc
    if ( psz == 8 ) {
      rns = deepPatchMatchMultiStart( dnz, reoTemplate, mask2, mask, psz, psz,
       block_name='ripmmarc', patchVarEx=0.98, meanCenter=FALSE, numberOfStarts=10  )
     } else {
       rns = deepPatchMatchMultiStart( dnz, reoTemplate, mask2, mask, psz, psz,
        block_name=20, patchVarEx=0.98, meanCenter=FALSE, numberOfStarts=10  )
     }
    wRansac = applyAntsrTransformToImage( rns$finalModel$transform,
        dnz, reoTemplate  )
    wpts = applyAntsrTransformToPoint( invertAntsrTransform( rns$finalModel$transform ),
      data.matrix(dnzpt ))
    imgListTestReo[[k]] = wRansac
    ptListTestReo[[k]] = wpts
    mpi=makePointsImage( wpts, wRansac )
    if ( TRUE ) {
      layout(matrix(1:3,nrow=1,byrow=T))
      plot( reoTemplate, wRansac, alpha=0.5 )
      plot(wRansac,iMath(mpi,"GD",4))
      mpi=makePointsImage( rns$movingPoints, dnz )
      plot(dnz,iMath(mpi,"GD",4))
      }
    }
  }
...
opre = paste0("models/autopointsupdate_relu_128_weights_3d_checkpoints5_GPU",mygpu)
...
###############################
generateData <- function( batch_size = 32, mySdAff=0.15, isTest=FALSE, verbose=FALSE, whichSample ) {
  if ( isTest ) {
  ...
  for ( looper in 1:batch_size ) {
    if ( missing( whichSample ) ) whichSample = sample(1:length(imgList),1)
    if ( isTest ) {
      whichSample=looper
    }
