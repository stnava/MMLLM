...
opre = paste0("models/mouse_rotation_3D_GPU",mygpu)
...
generateData <- function( batch_size = 32, mySdAff=0.15, isTest=FALSE, verbose=FALSE ) {
  ...
  for ( looper in 1:batch_size ) {
    ...
    locaff=mySdAff
    if ( isTest ) {
      temp = iMath(imgListTest[[whichSample]],"Normalize")
      blobs = ptListTest[[whichSample]]
      loctx = fitTransformToPairedPoints( blobs, ptTemplate, "Rigid" )$transform
      loctxi = invertAntsrTransform( loctx )
      rr = list( temp, loctx )
    } else {
      temp = iMath( imgList[[whichSample]], "Normalize" )
      blobs0 = ptList[[whichSample]]
...
#    loss =  SO3_6_param_loss )



# testing data inference
ggte = generateData(  mySdAff = 2, isTest=TRUE  )
